...
import { View, Text, StyleSheet, Image, TouchableOpacity, TextInput, useColorScheme, ScrollView } from 'react-native';
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      ...
      <View style={styles.searchContainer}>
        <View style={[styles.searchWrapper, { backgroundColor: isDark ? '#3a3a3aff' : '#FFFFFF' }]}>
          <Feather 
            name="search" 
            size={18} 
      ...
      <ScrollView style={styles.chatList} showsVerticalScrollIndicator={false}>
        ...
        {recentChats.map(section => (
          <View key={section.day}>
            <Text style={[styles.chatDay, { color: colors.text }]}>{section.day}</Text>
            {section.chats.map(chat => (
              <TouchableOpacity key={chat} style={styles.chatItem}>
                <Text style={[styles.chatText, { color: colors.text }]} numberOfLines={1}>
                  {chat}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <View style={[styles.footer, { borderTopColor: isDark ? '#333' : '#e0e0e0', paddingBottom: insets.bottom + 12 }]}>
        <View style={styles.profileRow}>
          <Image source={avatarSource} style={styles.avatar} />
          <View style={styles.profileInfo}>
            <Text style={[styles.profileName, { color: colors.text }]}>Sammy Njue</Text>
